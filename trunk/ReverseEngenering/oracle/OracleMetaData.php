...
class OracleMetaData implements DataSourceMetaData
{
	...
	// -------------------------------------------------------------------------
	function __construct($statments)
	{
		$this->columnCount = oci_num_rows($statments);
		for($i = 0; $i < $this->columnCount; $i++)
		{
			$col = new DataSourceColumnMetaData();
			$col->setName(oci_field_name($statments, $i));
			$col->setLength(oci_field_size($this->recordSet, $i));
			
			switch(oci_field_type($this->recordSet, $i))
			{
				case "TIMESTAMP":
				case "DATE":
					$col->setType("date");
