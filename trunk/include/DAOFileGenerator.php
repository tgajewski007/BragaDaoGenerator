...
class DAOFileGenerator
{
	...
	protected function generateGetAllForForeginColumn($t)
	{
		...
		foreach($t->getFk() as $fk) /* @var $fk ForeginKey */
		{
			...
			$this->addLine(" */", 1);
			$this->addLine("public static function getAllBy" . $fkTable->getClassName() . "(" . $fkTable->getClassName() . "DAO \$" .
	...
	protected function generateCreate(Table $t)
	{
		...
		if($pkSequenced)
		{
			$this->addLine("\$this->set" . ucfirst(current($pk)->getClassFieldName()) . "(\$db->getParam(\"" . mb_strtoupper(current($pk)->getName()) .
					"\"));", 3);
		}
		$this->addLine("\$db->commit();", 3);
		$this->addLine($t->getClassName() . "::updateFactoryIndex(\$this);", 3);
		$this->addLine("\$this->setReaded();", 3);
		$this->addLine("return true;", 3);
		$this->addLine("}", 2);
		$this->addLine("else", 2);
	...
	protected function generateGetterObject(Table $table)
	{
		foreach($table->getFk() as $fk)/* @var $fk ForeginKey */
		{
			...
			$this->addLine("{", 1);
			$this->addLine("if(is_null(\$this->" . lcfirst($fk->getTable()->getClassName()) . "))", 2);
			$this->addLine("{", 2);

			$this->addLine("\$this->" . lcfirst($fk->getTable()->getClassName()) . " = " . $fk->getTable()->getClassName() . "::get(" . implode(", ", $tmp1) .
					");", 3);
			$this->addLine("}", 2);
		...
		foreach($this->project->getTables() as $t) /* @var $t Table */
		{
			foreach($t->getFk() as $fk)/* @var $fk ForeginKey */
			{
				if($fk->getTable() == $table)
				{
					...
					$this->addLine("{", 1);
					$this->addLine("if(is_null(\$this->" . lcfirst($t->getClassName()) . "s))", 2);
					$this->addLine("{", 2);
					$this->addLine("\$this->" . lcfirst($t->getClassName()) . "s = " . $t->getClassName() . "::getAllBy" . ucfirst($table->getClassName()) .
							"(\$this);", 3);
					$this->addLine("}", 2);
					$this->addLine("return \$this->" . lcfirst($t->getClassName()) . "s;", 2);
					$this->addLine("}", 1);
					$this->addLine("// -------------------------------------------------------------------------", 1);
				}
			}
	...
	protected function generateSetter(Column $c)
	{
		...
		switch($c->getType())
		{
			...
			case ColumnType::DATE:
				$this->addLine("if(!is_null(\$" . $c->getClassFieldName() . "))", 2);
				$this->addLine("{", 2);
	...
	protected function generateConstruktor(Table $t)
	{
		...
		$this->addLine("{", 3);
		$this->addLine("throw new Exception(\"" . $t->getErrorPrefix() . "01 \" . " . $t->getSchema() . " . \"." . $t->getName() . "(\" . " .
				implode(" . \", \".", $tmp3) . " . \")  nie istnieje\");", 4);
		$this->addLine("}", 3);
		$this->addLine("}", 2);
		$this->addLine("}", 1);
		$this->addLine("// -------------------------------------------------------------------------", 1);
