...
class DAOFileGenerator
{
	...
	protected function generateStaticGetForUpdateMethod(Table $table)
	{
		...
		$this->addLine("else", 2);
		$this->addLine("{", 2);
		$this->addLine("throw new BragaException(\"" . $table->getErrorPrefix() . "05 Empty or wrong object id type\");", 3);
		$this->addLine("}", 2);
		$pk = array();
		foreach($table->getColumny() as $column)
		{
		...
		$this->addLine("else", 2);
		$this->addLine("{", 2);
		$this->addLine("throw new BragaException(\"" . $table->getErrorPrefix() . "06 " . $table->getName() . "(\" . " . implode(" . \", \".", $tmp3) . " . \")  does not exists\");", 3);
		$this->addLine("}", 2);
		$this->addLine("}", 1);
		$this->addLine("// -----------------------------------------------------------------------------------------------------------------", 1);
	}
	...
	protected function generateUpdate(Table $t)
	{
		...
		$this->addLine("{", 2);
		$this->addLine("\\braga\\graylogger\\BaseLogger::exception(new BragaException(\"SaveWithoutLock\", 0), \\Monolog\\Level::Critical, [ \"id\" => \$this->getKey(), \"class\" => \"" . $t->getClassName() . "\" ]);", 3);
		$this->addLine("}", 2);
		$this->addLine("\$db = new DB();", 2);
		$this->addLine("\$sql = <<<SQL", 2);
	...
	protected function generateSetter(Column $c)
	{
		...
		switch($c->getType())
		{
			...
			case ColumnType::NUMBER:
				...
				$this->addLine("{", 2);
				$this->addLine("\$this->" . $c->getClassFieldName() . " = intval(\$" . $c->getClassFieldName() . ");", 3);
				$this->addLine("}", 2);
				$this->addLine("else", 2);
				$this->addLine("{", 2);
				$this->addLine("\$this->" . $c->getClassFieldName() . " = null;", 3);
	...
	protected function generateConstruktor(Table $t)
	{
		...
		$this->addLine("{", 3);
		$this->addLine("throw new BragaException(\"" . $t->getErrorPrefix() . "01 " . $t->getName() . "(\" . " . implode(" . \", \".", $tmp3) . " . \")  does not exists\");", 4);
		$this->addLine("}", 3);
		$this->addLine("}", 2);
		$this->addLine("}", 1);
		$this->addLine("// -----------------------------------------------------------------------------------------------------------------", 1);
